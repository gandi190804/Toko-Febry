...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Handle file upload
    $foto_user = ''; // Inisialisasi nama file foto
    if ($_FILES['foto_user']['name']) {
        ...
        if (move_uploaded_file($_FILES["foto_user"]["tmp_name"], $target_file)) {
            // Jika berhasil, tampilkan pesan dalam variabel untuk ditampilkan nanti
            $upload_success = true;
        } else {
            // Jika gagal, tampilkan pesan kesalahan dalam variabel untuk ditampilkan nanti
            $upload_success = false;
        }
    }

    // Lakukan penambahan data admin ke dalam database
    $query = "INSERT INTO users (id, nama, username, password, no_hp, foto_user) VALUES (uuid(), '$nama', '$username', '$password', '$no_hp', '$foto_user')";
                ...
                <div class="w-100 d-block">
                    <div class="row justify-content-between">
                        <div class="col-auto d-flex align-items-center gap-3">
                            ...
                            <h1 class="m-0" style="font-weight: 600;">Tambah Admin</h1>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="p-3 mt-3">
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            if (isset($upload_success)) {
                if ($upload_success) {
                    echo "<div class='alert alert-success'>File " . htmlspecialchars(basename($_FILES["foto_user"]["name"])) . " berhasil diunggah.</div>";
                } else {
            ...
            if (isset($db_success)) {
                if ($db_success) {
                    echo "<script>alert('Admin berhasil ditambahkan!'); window.location.href='pengaturan.php';</script>";
                } else {
                    echo "<div class='alert alert-danger'>Gagal menambahkan admin!</div>";
                }
            }
        }
        ?>
